...
- __用途：通过自动化脚本参与JD商城的各种活动从而获取京豆用于购物抵扣，还可通过某些活动白嫖商品__
...
#### 4. 两版本的《部署教程》与《使用教程》均不相同，不要重复、错误使用，请认真阅读所有教程内容
...
5. 若使用 `VPS` 平台，部署前请进入您所使用平台提供商的防火墙功能，检查是否已开放相关端口、允许`HTTP/HTTPS`流量通过等设置。
...
3. `控制面板` 功能的初始用户名为 `useradmin`，初始密码为 `supermanito`，部署成功后可通过 `http://内部或外部IP地址:5678` 访问
4. 如果您的网络环境暴露在了公网（例如 VPS 用户），请根据《使用与更新》教程更改 `控制面板` 的用户名和密码。
