...
ㅤ
- __2021/2/28 17:00 `通知`__
ㅤDocker 版本即将迎来重大更新，全新构建了镜像，支持 arm/arm64/amd64 架构的设备，感谢某位大佬提供的技术支持，敬请期待......
...
#### __初始化容器：__
    docker logs -f jd
> _注意：请先执行此命令查看初始化容器进度，当输出`容器启动成功字样即代表容器创建成功`，此时通过命令 `Ctrl + C` 退出即可。_
#### __拉取脚本：__
