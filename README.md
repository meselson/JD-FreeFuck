...
已找到新的解决方法，重回组织怀抱，正在紧张制作中，即将到来......

- __2021/2/21 通知__

由于活动脚本作者lxk0301大佬的库因为滥用Gitee资源被官方封禁，导致无法更新也无法部署此项目，目前库已经解封变成私有状态，大佬正在寻找解决办法，已部署的用户暂时不受影响......

- __2021/2/20 通知（重要）__

由于活动脚本作者lxk0301大佬更新了`package.json`，导致已部署的朋友在19号更新后所有脚本报错并提示“请先获取JD账号”，请所有已部署的朋友根据《使用于更新》中`如何卸载此项目`的第一条命令删除整个项目文件夹后重新一键部署！\
\
注意删除整个项目文件夹前请先备份您的Cookie、互助码等配置信息，配置文件`config.sh`已更新到最新的版本。
...
3. 由于某些组件的安装受国外网络因素影响，如果部署失败请再次尝试，否则请严格按照模板提交至[ Issues ](https://github.com/SuperManito/JD-FreeFuck/issues)寻求帮助
4. 下方`PC 环境`与`VPS 环境`对应两种部署方案，区别在于是否使用了国内更新源加速下载，根据您的使用环境选择其一即可，不要重复部署
- __PC 环境：__
