...
- __2021/3/08 16:00 `更新`__
ㅤ更新了配置文件，新增 "定义京小兑是否将抽奖卷自动兑换为兑币" 环境功能控制变量，修正了部分注释内容.可根据对应版本《使用与更新》教程之<更新教程>的第2命令手动更新配置文件。

- __2021/3/08 12:30 `通知`__
ㅤ由于 7日 晚更新了一键更新脚本中的相关代码，导致 `Docker` 版用户无法启动容器，目前已修复此问题，请启动容器失败的朋友删除容器后重新启动容器即可。已部署的用户不受任何影响，请忽略此通知。

- __2021/3/08 01:45 `更新`__
ㅤ同步上游源码更新了控制面板，新增手动执行脚本功能。`Docker` 版用户可直接使用，`Linux` 版用户请根据《使用与更新》教程之<更新教程>的第3命令手动更新。

- __2021/3/07 23:00 `更新`__
ㅤ同步上游源码更新了控制面板，新增查看脚本文件功能。请所有朋友根据对应版本《使用与更新》教程之<控制面板教程>的第3条命令重启控制面板。
...
> _注意：此镜像大约需要占用 `239MB` 的空闲存储空间，目前最新版本的镜像ID为 `68e4d721f0fa` 。_
#### __启动容器：__
    docker run -dit \
