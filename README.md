...
- __2021/3/07 04:30 `通知`__\
ㅤ更新了一键更新脚本，新增同步项目Shell脚本功能，从而减少未来项目更新后的手动操作；修复了在启用diy脚本后重复输出删除定时任务的问题。已推送新版 `Docker` 镜像，建议重新部署，新版本将使用国内码云Gitee分项同步源码，如果您不想重新部署，可根据对应版本《使用与更新》教程之<更新教程>的第1条命令更新一键更新脚本，将继续使用Github同步项目Shell脚本。本项目基础功能已日益完善，未来将着手开发新的功能以提高您的体验，感谢您的使用与支持。
...
ㅤ去除了 `manual-update` 脚本，将其功能移入至 `git_pull` 原版一键更新脚本中，更新了对应教程注意今后使用新命令，配置文件新增同步 `Diy` 脚本功能，已推送新版 `Docker` 镜像且支持了更多的架构，最新版本的镜像ID为 `2f6bf791cf7c` ，如果您不想重新部署可通过对应版本《使用与更新》教程之<更新教程>的第3条命令升级现有版本，此次更新内容较少，主要为项目结构调整，感谢您的理解与支持。
...
> _注意：此镜像大约需要占用 `239MB` 的空闲存储空间，目前最新版本的镜像ID为 `443a9243c32e` 。_
#### __启动容器：__
    docker run -dit \
