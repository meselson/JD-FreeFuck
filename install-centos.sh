...
#适用系统: CentOS 7.0 ~ 8.3，包括所有子版本号系统，如果是最小化安装，请通过SSH方式进入到终端
...
if [ $VERSION -eq 7 ];then
  ...
  yum makecache
  yum install -y net-tools git curl perl moreutils gcc gcc-c++
  wget https://mirrors.tuna.tsinghua.edu.cn/nodejs-release/v14.14.0/node-v14.14.0-linux-x64.tar.gz
  tar -zxvf node-v14.14.0-linux-x64.tar.gz -C /opt
  mv /opt/node-v14.14.0-linux-x64 /opt/nodejs
  ln -s /opt/nodejs/bin/node /usr/local/bin/
  ln -s /opt/nodejs/bin/npm /usr/local/bin/
  rm -rf node-v14.14.0-linux-x64.tar.gz
fi
if [ $VERSION -eq 8 ];then
  ...
  yum install -y net-tools git curl nodejs npm perl moreutils
fi
}
